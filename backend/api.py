...
class ProductPinnedUpdate(BaseModel):
    pinned: bool
...
@app.patch("/api/products/{product_id}/pinned")
def update_single_product_pinned(product_id: int, payload: ProductPinnedUpdate):
    """Atualiza o status de favorito/fixado de um produto."""
    prod = get_product(product_id)
    if not prod:
        raise HTTPException(status_code=404, detail="Produto não encontrado.")
    
    update_product_pinned(product_id, payload.pinned)
    return {"message": "Status de favorito do produto atualizado com sucesso.", "pinned": payload.pinned}
